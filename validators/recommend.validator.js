...
const recommendValidator = {
    ...
    recommendSchema: Joi.object({
        location: Joi.string().required().messages({ 'any.required': '위치 정보는 필수입니다.' }),
    ...
    complexSearchSchema: Joi.object({
        lat: Joi.number().min(-90).max(90).required(),
        lng: Joi.number().min(-180).max(180).required(),
...
module.exports = recommendValidator;
